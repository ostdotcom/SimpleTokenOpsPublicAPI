...
const web3RpcProvider = require('../web3/rpc_provider')
    ...
    , getStartIndexABI = currContract.methods.process(0).encodeABI()
    , fs = require('fs');

const _private = {

  processGetAddressesResponse: async function (getAdressesResponses) {

    var addressesArray = await helper.toAddressArray(getAdressesResponses)
    , bonusesData = [];
...
const bonusesContractInteract = {
  ...
  writeBonusesDataToCsv: async function (fileName) {
    ...
    var getAdressesResponse = await helper.call(getAddressesABI, currContractAddress);

    var bonusesData = await _private.processGetAddressesResponse(getAdressesResponse);
